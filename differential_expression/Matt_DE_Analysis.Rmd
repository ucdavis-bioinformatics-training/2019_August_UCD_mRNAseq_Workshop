...
MATT SAYS TO DO THIS HERE!
```{r, warnings=FALSE}
library(edgeR)
library("RColorBrewer") # this is for coloring
# and this for plotting
library("gplots")
```
...
Common filter is to remove genes with a max value (X) in less than Y samples.
...
#drop <- which(rownames(d0$counts) %in% c("ENSG00000223972.5","ENSG00000227232.5","ENSG00000238009.6","ENSG00000268903.1","ENSG00000269981.1","ENSG00000241860.6"))
d <- d0[-drop,] 
...
```{r}
plot_coord <- plotMDS(d, col = as.numeric(group))
```
...
top.table <- top.table[,c("Gene", names(top.table)[c(1,2,4,5)])]

gene_map <- match(sapply(strsplit(top.table$Gene,split="\\."),"[[", 1L),sapply(strsplit(anno$Gene.stable.ID.version,split="\\."),"[[", 1L))
      
top.table <- data.frame(top.table,anno[gene_map,],logcpm[match(top.table$Gene,rownames(logcpm)),])
head(top.table)
...
```{r}
## Interaction term
#contr <- makeContrasts( (groupA.C - groupB.C) - (groupA.D - groupB.D), levels = colnames(coef(fit)))
#contr <- makeContrasts( (groupA.C - groupA.D) - (groupB.C - groupB.D), levels = colnames(coef(fit)))

contr <- makeContrasts(groupA.C - groupB.C, groupA.C - groupA.D, levels = colnames(coef(fit)))
tmp <- contrasts.fit(fit, contr)
tmp <- eBayes(tmp)
...
heatmap.2(logcpm[rownames(top.table),],col=brewer.pal(11,"RdBu"),labRow=anno2[match(sapply(strsplit(rownames(top.table),split="."),"[[",1L),sapply(strsplit(rownames(top.table),split="."),"[[",1L)),"Gene.name"], scale="row", trace="none")
```
