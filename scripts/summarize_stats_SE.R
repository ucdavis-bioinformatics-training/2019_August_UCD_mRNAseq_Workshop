...
check_apps <- c("hts_Stats", "hts_SeqScreener", "hts_SeqScreener.1", "hts_AdapterTrimmer", "hts_QWindowTrim", "hts_NTrimmer", "hts_CutTrim", "hts_Stats.1")
...
QwinTotalSELT <- sapply(hts_json,function(x) unlist(sapply(x[5],"[[","Single_end")[c("SE_leftTrim"),]))
QwinTotalSERT <- sapply(hts_json,function(x) unlist(sapply(x[5],"[[","Single_end")[c("SE_rightTrim"),]))
QwinTotalDiscard <- sapply(hts_json,function(x) unlist(sapply(x[5],"[[","Single_end")[c("SE_discarded"),]))

## N trim
NwinTotalReadsIn <- sapply(hts_json,function(x) sapply(x[6], "[[","totalFragmentsInput"))
NwinTotalSELT <- sapply(hts_json,function(x) unlist(sapply(x[6],"[[","Single_end")[c("SE_leftTrim"),]))
NwinTotalSERT <- sapply(hts_json,function(x) unlist(sapply(x[6],"[[","Single_end")[c("SE_rightTrim"),]))
NwinTotalDiscard <- sapply(hts_json,function(x) unlist(sapply(x[6],"[[","Single_end")[c("SE_discarded"),]))

## CutTrim
CutTrimTotalReadsIn <- sapply(hts_json,function(x) sapply(x[7], "[[","totalFragmentsInput"))
CutTrimTotalDiscard <- sapply(hts_json,function(x) unlist(sapply(x[7],"[[","Single_end")[c("SE_discarded"),]))
...
stats2TotalReadsSE_BpLen <- sapply(hts_json,function(x) unlist(sapply(x[8],"[[","Single_end")[c("SE_bpLen"),]))
stats2TotalReadsSE_BQ30 <- sapply(hts_json,function(x) unlist(sapply(x[8],"[[","Single_end")[c("SE_bQ30"),]))
